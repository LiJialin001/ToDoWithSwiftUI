                ...
                HStack(spacing:20) {
                    ...
                    Button(action:{
                        ...
                        if title != "" {
                            ...
                            self.database.ToDos.append(ToDoItem(title: self.title, priorityLevel: self.priority ,classification: self.classify, date: self.date, lastDate: self.date2, isPersonal: true, isLoop: isLoop ,isStop: isStop, cyclePeriod: loopTime, Team: DetailItem))
                            self.databaseMessage.Messages.append(MessageItem( title: self.title, mainMessage: "来自团队 \(DetailItem.name) 的待办", itsTeam: DetailItem, classification: 1, date: Date(), isRead: false))
                        }
                    }){
